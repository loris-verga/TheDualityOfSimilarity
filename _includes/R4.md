...
<p>
Plan: (1) compare single-feature models, (2) test a combined model, (3) look at robustness under class imbalance, and (4) compare logistic regression with a Random Forest approach.
</p>

<hr style="margin:18px 0;">

<h3><span style="color:#ff201e">4.1 Comparing predictive power (overview)</span></h3>

<p>
We study three single-feature classifiers (one per distance metric), a combined classifier using all three distances, and two model families: <em>Logistic Regression</em> and <em>Random Forest</em>.
...
<h3><span style="color:#ff201e">4.2 Logistic regression — formulas & setup</span></h3>
...
<h3><span style="color:#ff201e">4.2 Results — logistic regression (imbalanced data)</span></h3>
...
<div style="margin-top:12px; font-size:0.95rem;">
  ...
  <ul>
    <li>Shared authorship (authors_cos_dist) is the strongest single predictor, though an AUC around 0.60 still suggests limited accuracy overall.</li>
    <li>Psychological distance provides a modest but steady signal (AUC around 0.58).</li>
...
<h3><span style="color:#ff201e">4.3 Downsampling (balanced classes)</span></h3>
...
<div style="margin-top:12px;">
  <p><b>Takeaway:</b> balancing the dataset boosts metrics tied to class ratios (precision, recall, F1) but does not change how well the model actually separates positive from negative links, as shown by the nearly unchanged ROC-AUC.</p>
</div>

<hr style="margin:18px 0;">


<h3><span style="color:#ff201e">4.4 Logistic regression — combined models</span></h3>

<p>We compare two logistic models: (A) using all three distances, and (B) using only authorship + psychological distance, leaving out stylometric.</p>
...
<h3><span style="color:#ff201e">4.5 Random Forest — combined model</span></h3>
...
<h3><span style="color:#ff201e">4.6 Feature importance & stylometric usefulness</span></h3>
...
<h3><span style="color:#ff201e">4.7 Final conclusions (Part 4)</span></h3>

<div style="background:#eef1f5; padding:16px; border-radius:10px;">
  <ol>
    <li><b>H11 confirmed:</b> shared authorship distance is the most predictive individual metric (highest AUC and F1).</li>
    <li><b>H12 confirmed:</b> combining distances improves performance. Logistic regression reaches around 0.615 AUC, while Random Forest pushes that up to about 0.723.</li>
    <li><b>H13 not supported:</b> stylometric distance is weak alone, but it still adds a small, steady improvement when combined with the others.</li>
  </ol>
