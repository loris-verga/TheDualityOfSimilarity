...
<p>
We've explored, correlated, and even played causal detective. Now it's time for the ultimate showdown: which of our three similarity metrics is the best crystal ball for predicting Reddit negativity? Can we actually guess whether a hyperlink will be friendly or negative before it's even posted?
</p>
---

## <span style="color:#ff4500">Predictive Power of Link Sentiment</span>
...
<h4><span style="color:#ff4500">Predictive power overview</span></h4>

<p>
We study three single-feature classifiers (one per distance metric) as well as a combined classifier using all three distances that could yield better results.<br><br>
...
<h4><span style="color:#ff4500"> Logistic regression, what are the key formulas & setup ?</span></h4>
...
<div style="display:flex; flex-direction: row; justify-content: center; align-items: flex-start; gap:16px; flex-wrap: nowrap; margin-top:12px; width:100%;">
  <details style="position:relative; width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000;">
    <summary>F1 score</summary>
    The F1 score is a single metric used to evaluate classification performance by balancing precision and recall, especially useful when classes are imbalanced. <br><br>
			\( \text{F1} = 2 \cdot \frac{\text{Precision}\cdot \text{Recall}}{\text{Precision} + \text{Recall}} \)
  </details>

  <details style="width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000">
    <summary>Precision & Recall</summary>
    Precision measures how many predicted positives are truly positive, while recall measures how many actual positives are correctly identified by the model. <br><br>
  ...
  <details style="width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000">
    ...
    Logistic regression is a classification model used to estimate the probability of a binary outcome as a function of explanatory variables. <br><br>
      \( \mathbb{P}(y=1|X) = \sigma(\beta_0 + \beta_1\cdot x1 + \beta_2\cdot x2 + \beta_3\cdot x3 + ...) \) <br>
...
<h4><span style="color:#ff4500">4.3 Results : Logistic regression with imbalanced data</span></h4>

<div style="display:flex; flex-direction: row; justify-content: center; gap: 18px; flex-wrap: nowrap; margin-top:8px;">
  <div style="background:#fff; border:1px solid #e6e9ec; padding:12px; border-radius:8px; font-size:19px; width:100%;">
...
<div style="display:flex; flex-direction: row; justify-content: center; align-items: flex-start; gap:16px; flex-wrap: nowrap; margin-top:12px; width:100%;">
  <details style="position:relative; width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000;">
    <summary>Psychological distance</summary>
    F1: <b>0.206 ± 0.001</b><br>
    AUC: <b>0.584 ± 0.003</b>
  </details>

  <details style="width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000">
    <summary>Authors distance</summary>
    F1: <b>0.219 ± 0.001</b><br>
    AUC: <b>0.606 ± 0.002</b>
  </details>

  <details style="width:100%; background:#f6f9fa; padding:12px; border-radius:8px; color: #000000">
    <summary>Stylometric distance</summary>
    F1: <b>0.171 ± 0.002</b><br>
    AUC: <b>0.511 ± 0.003</b>
  </details>
...
<h4><span style="color:#ff4500">4.4 Downsampling with balanced classes</span></h4>
...
<strong>Downsampled results</strong>
<div style="display:flex; gap:12px; margin-top:10px;">

  <div style="background:#fff; border:1px solid #e6e9ec; padding:12px; border-radius:8px; font-size:19px; width:100%;">
    <strong>Psychological distance</strong><br><br>
    F1 <b>0.569 ± 0.001</b><br>
    AUC <b>0.583 ± 0.002</b><br>
  </div>

  <div style="background:#fff; border:1px solid #e6e9ec; padding:12px; border-radius:8px; font-size:19px; width:100%;">
    <strong>Authors distance</strong><br><br>
    F1 <b>0.589 ± 0.002</b><br>
    AUC <b>0.604 ± 0.001</b><br>
  </div>
...
<div style="background:#fff7ef; padding:12px; border-radius:8px; font-size:19px; margin-top: 10px;">
...
<h4><span style="color:#ff4500">4.5 Logistic regression : combined models</span></h4>
...
<h5><span style="color:#ff4500">Superiority of predictive power of the combined model</span></h5>
...
<h4><span style="color:#ff4500">4.6 Random Forest : combined model</span></h4>
...
<div style="display:flex; flex-direction: row; gap:12px; margin-top:10px;">
  <div style="background:#fff; border:1px solid #e6e9ec; padding:12px; border-radius:8px; width:60%;">
  ...
  <div style="background:#f0fff4; padding:12px; border-radius:8px; width:40%;">
...
<div style="background:#f7f9fc; padding:20px; border-radius:10px; line-height:1.6; font-size:15px; margin-top:10px;">
...
<p>Finally, we plot the ROC curve to visualize the classifier's performance and the AUC score.</p>

<div style="text-align:center; margin-top:14px;">
  <img src="assets/img/R4/rf_roc_curve.png" width="50%" alt="Random forest ROC curve">
  <div style="font-size:0.85rem;"><em>ROC curve with an AUC of about 0.723 for the Random Forest.</em></div>
</div>

<hr style="margin:18px 0;">

<h4><span style="color:#ff4500">4.7 Feature importance & stylometric usefulness</span></h4>
...
<h4><span style="color:#ff4500">4.8 Final conclusions</span></h4>
...
<div style="background:#eef1f5; padding:16px; border-radius:10px;">
  <ol>
    ...
    <li>Is stylometric distance totally useless on the prediction when included in the combined predictive model ? <br>
    <b>Not supported !</b> stylometric distance is weak alone, but it still adds a small, steady improvement when combined with the others.</li>
  </ol>

  <p style="margin-top:8px; margin-bottom:0;">
    <b>Practical takeaway:</b> authorship distance and psychological distance together form a solid starting point.  
